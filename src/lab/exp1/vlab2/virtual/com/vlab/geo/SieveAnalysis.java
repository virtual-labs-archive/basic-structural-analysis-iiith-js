...
public class SieveAnalysis extends javax.swing.JPanel {
	...
	private SieveAnalysisBody       freeBody =null;               // Shape3D
	...
	public BranchGroup createSceneGraph() {
		...
		tg.setTransform(t);
		freeBody = new SieveAnalysisBody();    
	 ...
	 */
	public SieveAnalysis(Container container) {
	...
	public static class MyApplet extends JApplet {
		...
		SieveAnalysis mainPanel;

		public void init() {
			setLayout(new BorderLayout());
			mainPanel = new SieveAnalysis(this);
	...
	private static class MyFrame extends JFrame {
		...
		MyFrame() {
			...
			setTitle("Grain Size Applet");
			getContentPane().add(new SieveAnalysis(this), BorderLayout.CENTER);
			pack();
		}
	}
...
//
	...
	private void centerPanel(Container container) {
		...
		JLabel lbl = new JLabel("Sieve Analysis", JLabel.CENTER);
		...
		resultSet=freeBody.init(BOS,(int)fields[1]);
		          for(int i1=0;i1<10;i1++)
		        	  for (int j1=0;j1<5;j1++)
		        		  table.setValueAt(resultSet[i1][j1],i1,j1);
		table = new JTable(cellData, columnNames);
		jsp = new JScrollPane(table);
...
class SieveAnalysisBody 
 {
	...
	public String[][] init(String datafile,int mass)
	{
	 ...
	 //String data_file = "resources/Sieve/" + datafile + "/data.txt";
