...
public class DefBeams extends javax.swing.JPanel {
	...
	public BranchGroup createSceneGraph() {
		...
		objRoot.addChild(m_j3d.createBox(new Vector3d(0, -0.25, -.1),
	...
	private Group createVirtualLab() {
		...
		// brick
		// table
		//objtrans.addChild(m_j3d.createTextureBox("resources/images/tile1.jpg",
		// new Vector3d(0,0.4, 0.2),new Vector3d(0.5,.04,.01), new
		// Vector3d(0,0,0),new Color3f(1f, 1f, 1f)));
		// objtrans.addChild(loadObjectFile(
		// "resources/geometry/sixDoorFrenchStyle.obj",new Vector3d(0,0.17,
		// 0.3),new Vector3d(.15,.25,.5), new Vector3d(0,0,0),new Color3f(0f,
		// 1f, 0f)));
		// objtrans.addChild(createTextureBox("resources/images/brick.jpg",new
		// Vector3d(0,0.1, 0),new Vector3d(0.4,.29,.01), new Color3f(1f, 1f,
		// 1f)));

		// Block Left
		//objtrans.addChild(m_j3d.createTextureBox("resources/images/tile1.jpg",
		// new Vector3d(-.68,0.17, -.1),new Vector3d(.2,.02,.3),new
		// Vector3d(0,0,0),new Color3f(1f, 1f, 1f),"roof1",hm));
		// /objtrans.addChild(m_j3d.createTextureCube(new Vector3d(-.7,-0.05,
		// -.1),new Vector3d(.3,.4,.3), new Vector3d(0,0,0),new Color3f(1f, 1f,
		// 1f),"resources/images/brick2.jpeg","block1",hm));
		// Block Center
		objtrans
				.addChild(m_j3d.createTextureBox(new Vector3d(0, 0.17, -.1),
						new Vector3d(.2, .02, .3), new Vector3d(0, 0, 0),
						new Color3f(1f, 1f, 1f), "resources/images/tile1.jpg",
						"roof1", hm));
		objtrans.addChild(m_j3d.createTextureCube(new Vector3d(0, -0.05, -.1),
				new Vector3d(.3, .4, .3), new Vector3d(0, 0, 0), new Color3f(
						1f, 1f, 1f), "resources/images/brick2.jpeg", "block1",
				hm));
		// Block Right
		//objtrans.addChild(m_j3d.createTextureBox("resources/images/tile1.jpg",
		// new Vector3d(.68,0.17, -.1),new Vector3d(.2,.02,.3),new
		// Vector3d(0,0,0),new Color3f(1f, 1f, 1f),"roof3",hm));
		// objtrans.addChild(m_j3d.createTextureCube(new Vector3d(.7,-0.05,
		// -.1),new Vector3d(.3,.4,.3), new Vector3d(0,0,0),new Color3f(1f, 1f,
		// 1f),"resources/images/brick2.jpeg","block3",hm));
	...
	private void rightPanel() {
		...
		rightBottom.setBorder(BorderFactory.createLineBorder(new Color(140,
				200, 240), 8));
	...
	private void startSimulation(java.awt.event.ActionEvent evt) {
		...
		if (valChange) {
			...
			// init(double len,double bre,double dep,double matGd,double
			// safFac,int endC)
	...
	private void timerActionPerformed(java.awt.event.ActionEvent evt) {
		...
		if (String.valueOf(freeBody.shearForce()).length() > 4)
			outlbl_val[i++].setText(String.valueOf(freeBody.shearForce())
					.substring(0, 7)
					+ " kN");

		if (String.valueOf(freeBody.bendMnt()).length() > 4)
			outlbl_val[i++].setText(String.valueOf(
					freeBody.bendMnt()).substring(0, 7)
					+ " kN");

		if (String.valueOf(freeBody.getMoment()).length() > 4)
			outlbl_val[i++].setText(String.valueOf(freeBody.getMoment())
					.substring(0, 7)
					+ " kN");

//		if (String.valueOf(freeBody.getSlenderNessLoad()).length() > 4)
//			outlbl_val[i++].setText(String.valueOf(
//					freeBody.getSlenderNessLoad()).substring(0, 7)
//					+ " ");
//
//		len.setText(String.valueOf(freeBody.getLength()));

		java.util.BitSet visibleNodes = new java.util.BitSet(switchGroup1
				.numChildren());
		...
		// outlbl_val[i++].setText(String.valueOf(freeBody.getCriticalLoad()) +
		// " kN");
		//outlbl_val[i++].setText(String.valueOf(freeBody.getRankeesCriticalLoad
		// ()) + " kN");
		// outlbl_val[i++].setText(String.valueOf(freeBody.getAxialLoad()) +
		// " kN");
	...
	private void pauseSimulation() {
		...
		timer.stop();
		java.util.BitSet visibleNodes = new java.util.BitSet(switchGroup4
				.numChildren());
		switchGroup4.setChildMask(visibleNodes);
...
//	}
	...
	public void init(double d,double bre,double dep, int jMain, int iSub, double W, double f,double W1) {
		                ...
		                case 3:
		                	for(double i=0;i<length;i=i+50.0)
		                     { 
		                		  ...
		                		  BM.add(-(W*(length-i)*(length-i)/2 + W1*(length-i)));
		                     }
		                	 break;
		                case 4:
		                	for(double i=0;i<length;i=i+50.0)
		                     { 
		                		  ...
		                		  BM.add(-W*(length-i)*(length-i)*(length-i)/(6*length));
		                     }
		                	 break;
		                case 5:
		                	for(double i=0;i<length;i=i+50.0)
		                     { 
		                		  ...
		                		  BM.add(W*length*i/2-W*i*i*i/(6*length)-W*length*length/3); // W*L*i/2 - W*i^3/(6*L) - W*L^2/3;
		                		//  System.out.println("SF = " + SF + "BM = " + BM );  
		                     }
		                	 break;
...
// }
//	
// void init(double len,double bre,double dep,double matGd,double safFac,int
// endC)
// {
// // System.out.println("length :" + len + " Breadth " + bre + "depth " + dep +
// "matgd " + matGd + "safty Factor" + safFac + "end c " + endC );
// length=len*1000;
//        
// breadth=bre;
// depth=dep;
...
// if(endC==1)
// {
//Y=(((W*length*x*x*x)/12)-((W*x*x*x*x)/24)-((W*length*length*length*x)/24))/(E*
// I);
// return Y;
// }
// if(endC==2)
// {
// Y=(((-W*(length-x)*(length-x)*(length-x)*(length-x))/24)-((W*length*length*
// length*x)/6)+(W*length*length*length*length)/24)/(E*I);
// return Y;
// }
// if(endC==3)
// {
//Y=(W*length*x*x*x/12.0-W*x*x*x*x/24.0-W*length*length*x*x/32.0-W*length*length
// *length*x/96.0)/(E*I);
//			
