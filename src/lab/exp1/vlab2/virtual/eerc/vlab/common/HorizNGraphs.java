...
public class HorizNGraphs extends JPanel implements MouseListener, MouseMotionListener 
{
	private Image mImage;
	int 	m_numOfGraphs=0; // number of graphs
	int 	m_counter=0;
	int 	m_state=0;
	...
	public HorizNGraphs (int w,int h,int numOfGraphs,String str_x,String str_y) {
		   //setBackground(Color.black);
		   
		   
		   m_numOfGraphs = numOfGraphs;
		   m_width = w;
		   m_height = h;
		   m_x_str=str_x;
		   m_y_str=str_y;
		   ...
		   m_fit_yWindow = false;
		   m_Vdisplay = false;
		   m_nColor = new Color[m_numOfGraphs];
		   for(int i=0; i<m_nColor.length;i++)
			   m_nColor[i] = Color.blue;
		   
		   m_nCaptions= new String[m_numOfGraphs];
		   m_nValues= new float[m_numOfGraphs];
		   m_nUnits= new String[m_numOfGraphs];
	...
	public HorizNGraphs (HorizNGraphs graph,int w,int h) {
		   //setBackground(Color.black);
	...
	public void createGraph(Graphics g)
	{
	    ...
	    int i=m_counter+m_numOfGraphs;	// initially counter is 0;
	    int x=m_x_offset;
	    for(; x < m_width && i<m_vrtx.size()-m_numOfGraphs ; x++){
	    	for(int n=0;n<m_numOfGraphs;n++,i++){
	    		 g.setColor(m_nColor[n]);
	    		 g.drawLine(x,m_y_offset + (int)(m_vrtx.get(i-m_numOfGraphs)*m_scale_y), x,m_y_offset+(int)(m_vrtx.get(i)*m_scale_y));
	    ...
	    g.drawLine(m_x_offset, m_y_offset - (int)(m_height*0.4), m_x_offset, m_y_offset + (int)(m_height*0.4));
	...
	private void drawAllText(Graphics g){
		...
		for(i=0;i<m_numOfGraphs;i++){
			g.drawString( m_nCaptions[i] + " : " + m_nValues[i]  +  " " +  m_nUnits[i], 35,m_height-20 -10*i);
		}
		g.drawString( m_x_str + " : " + m_cur_x  +  " " +  m_x_unit, 35,m_height-10);
		
		// Top sub captions
		for(i=0;i<m_numOfGraphs;i++){
			g.setColor(m_nColor[i]);
			g.drawString( m_nCaptions[i] + " : " , (int)(m_width*.7),40+ 10*i);
		}
	...
	private void showScale(Graphics g)
	{
	    ...
	    for(int y= m_y_offset-y_step ; y>m_y_offset - (int)(m_height*0.4) ;y-=y_step)
	    	g2d.drawLine(m_x_offset,y, m_width - m_x_offset,y);
	    for(int y= m_y_offset + y_step; y<m_y_offset + (int)(m_height*0.4);y+=y_step)
	    	g2d.drawLine(m_x_offset,y, m_width - m_x_offset,y);
	
	    for(int x= m_x_offset + y_step ; x<m_width - m_x_offset;x+=y_step)
	    	g2d.drawLine(x,m_y_offset - (int)(m_height*0.4), x,m_y_offset + (int)(m_height*0.4));
	    
	    Font cur_Font = new Font("Arial", Font.BOLD, 8);
		g.setFont(cur_Font);
		
	    int i=y_step;
	    for(int y= m_y_offset-y_step ; y>m_y_offset - (int)(m_height*0.4) ;y-=y_step, i+=y_step)
    ...
    public void addGraphValue (double val[]) {
    	...
    	for(int i=0;i<val.length;i++)
    		m_vrtx.add(new Float(-val[i]));
    	
    	if(m_vrtx.size()/m_numOfGraphs > m_width*0.7)
	    	m_counter+=m_numOfGraphs;
...
//	  }
	...
	public void setNColors(Color clr[])
	{
		m_nColor = clr;
	}
	
	public void setNCaptions(String captions[])
	{
		
		m_nCaptions= captions;
		 
	}
	public void setNCurrentValues(float vals[])
	{
	
		   m_nValues= vals;
	}
	public void setNUnits(String units[])
	{
